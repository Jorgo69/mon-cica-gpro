<div class="space-y-6">
    ...
    @if (session('status'))
        <div class="mb-6 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-bold uppercase tracking-wider">
            {{ session('status') }}
        </div>
    @endif
    ...
    <form wire:submit="login" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                <input id="remember_me" type="checkbox" wire:model="remember" class="w-4 h-4 text-accent border-slate-300 dark:border-slate-700 rounded focus:ring-accent/20 dark:bg-slate-900 transition-all cursor-pointer">
                <span class="ms-2 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider group-hover:text-slate-700 dark:group-hover:text-slate-200 transition-colors">{{ __('Remember me') }}</span>
            </label>
        ...
        <div class="relative py-4">
            ...
            <div class="relative flex justify-center text-[10px] uppercase tracking-[0.2em] font-black">
                <span class="bg-white dark:bg-slate-900 px-4 text-slate-400">Ou continuer avec</span>
            </div>
        </div>

        <div class="grid grid-cols-2 gap-4">
            <x-ui.button variant="outline" class="w-full !rounded-2xl !py-3 bg-white/50 dark:bg-slate-800/50">
