<div class="bg-gray-100 dark:bg-gray-800 p-6 rounded-lg shadow-lg">
    ...
    <form wire:submit.prevent="saveSubActivities">
        ...
        <div class="relative p-4 border rounded-lg mb-4 bg-gray-50 dark:bg-gray-700">
            ...
            <button type="button" wire:click="removeSubActivity({{ $index }})" class="absolute top-2 right-2 text-red-500 hover:text-red-700">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    <label for="start_date-{{ $index }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Date de début</label>
                    {{-- <input type="date" id="start_date-{{ $index }}" wire:model.live="subActivitiesData.{{ $index }}.start_date" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 shadow-sm focus:border-blue-500 focus:ring-blue-500"> --}}
                    <input type="date" id="start_date-{{ $index }}" wire:model.defer="subActivitiesData.{{ $index }}.start_date">
                ...
                <div>
                    ...
                    {{-- <input type="date" id="end_date-{{ $index }}" wire:model.live="subActivitiesData.{{ $index }}.end_date" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 shadow-sm"> --}}
                    <input type="date" id="end_date-{{ $index }}" wire:model.defer="subActivitiesData.{{ $index }}.end_date">
                    @error('subActivitiesData.'.$index.'.end_date') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                </div>
                <div>
