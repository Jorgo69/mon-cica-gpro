    ...
    <x-ui.page-header :title="__('calendar.title')" :subtitle="__('calendar.subtitle')">
        <x-slot:actions>
            <div class="flex items-center gap-2">
                <x-ui.button wire:click="exportIcal" variant="outline" icon="download" size="sm">
                    iCal
                </x-ui.button>
                {{-- Subscribe URL --}}
                <div x-data="{ showUrl: false, copied: false }" class="relative">
                    <x-ui.button @click="showUrl = !showUrl" variant="outline" icon="rss" size="sm">
                    ...
                    <div x-show="showUrl" @click.away="showUrl = false" x-cloak
                        ...
                        <div class="flex gap-2">
                            <x-ui.input type="text" value="{{ \App\Http\Controllers\IcalFeedController::getFeedUrl(auth()->user()) }}"
                                   readonly id="ical-url" class="text-[10px] flex-1" />
                            <button @click="navigator.clipboard.writeText(document.getElementById('ical-url').value); copied = true; setTimeout(() => copied = false, 2000)"
                                    class="px-2 py-1 bg-accent text-white text-[10px] font-bold rounded-lg shrink-0">
...
<div x-data="calendarApp(@js($events), '{{ $date->format('Y-m-d') }}', '{{ $viewMode }}')" class="space-y-4">
    ...
    <x-ui.card class="mb-4">
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            {{-- Filter --}}
            <x-ui.select wire:model.live="projectFilter" icon="folder">
