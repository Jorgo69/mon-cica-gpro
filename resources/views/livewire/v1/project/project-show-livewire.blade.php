    ...
    @if ($project)
        ...
        <x-ui.page-header :title="__('projects.show.title')" :subtitle="$project->project_code">
            <x-slot:actions>
                ...
                @endcan
                <livewire:v1.project.project-share-livewire :projectId="$project->id" />
            ...
            @if($activeTab === 'logframe')
                ...
                @if($activeTab === 'analytics')
                    <x-ui.section :title="__('indicators.tracking_title')" icon="target">
                        @livewire('v1.project.indicator-tracking-livewire', ['projectId' => $project->id])
                    </x-ui.section>
                @endif

                @if($activeTab === 'timeline')
                    @include('livewire.v1.project.include.gantt-timeline', ['project' => $project])
                @endif
                ...
                @if($activeTab === 'history')
                    ...
                    <x-ui.section :title="__('projects.show.audit_trail')" icon="history">
                        @livewire('v1.audit.activity-history-livewire', ['subject' => $project])
                    </x-ui.section>
                @endif
