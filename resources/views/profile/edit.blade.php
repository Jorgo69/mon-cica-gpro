<x-app-layout>
    <x-ui.page-layout>
        ...
        <x-ui.page-header :title="__('settings.profile.title')" :subtitle="__('settings.profile.subtitle')" />

        <div class="space-y-6">
            {{-- Informations du profil --}}
            {{-- Avatar --}}
            <x-ui.section :title="__('settings.profile.avatar')" icon="image" :noPadding="false">
                <div x-data="{
                        ...
                        <div>
                            ...
                            <div class="flex gap-2 mt-2">
                                <button type="button" @click="showPicker = !showPicker"
                                    ...
                                    {{ __('settings.profile.change_avatar') }}
                                </button>
                                <button type="button" x-show="current" @click="removeAvatar()"
                                    ...
                                    {{ __('settings.profile.remove_avatar') }}
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="showPicker" x-collapse x-cloak class="mt-6 pt-6 border-t border-border-light">
                        <p class="text-xs font-bold text-muted uppercase tracking-wider mb-3">{{ __('settings.profile.choose_avatar') }}</p>
                        ...
                        <p x-show="saving" class="text-xs text-accent font-bold mt-2">{{ __('common.loading') }}</p>
                    </div>
                </div>
            </x-ui.section>

            {{-- Informations personnelles --}}
            <x-ui.section :title="__('settings.profile.personal_info')" icon="user" :noPadding="false">
                <form method="post" action="{{ route('profile.update') }}" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label for="name" class="text-xs font-bold text-heading uppercase tracking-wider block mb-2">{{ __('settings.profile.full_name') }}</label>
                        ...
                        <div>
                            <label for="email" class="text-xs font-bold text-heading uppercase tracking-wider block mb-2">{{ __('settings.profile.email') }}</label>
                        ...
                        <div>
                            ...
                            <select id="sexe" name="sexe"
                                ...
                                <option value="Homme" {{ $user->sexe === 'Homme' ? 'selected' : '' }}>{{ __('common.male') }}</option>
                                <option value="Femme" {{ $user->sexe === 'Femme' ? 'selected' : '' }}>{{ __('common.female') }}</option>
                            </select>
                        </div>
                    ...
                    @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && !$user->hasVerifiedEmail())
                        <div class="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-200 dark:border-amber-800">
                            <p class="text-xs text-amber-700 dark:text-amber-300">
                                ...
                                <button form="send-verification" class="font-bold underline hover:text-amber-900">{{ __('Click here to re-send the verification email.') }}</button>
                            </p>
                    ...
                    <div class="flex items-center gap-3 pt-2">
                        ...
                        @if (session('status') === 'profile-updated')
                            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                                class="text-xs font-bold text-emerald-600">{{ __('settings.profile.saved') }}</p>
                        @endif
                    </div>
                </form>
            </x-ui.section>
            ...
            {{-- Mot de passe --}}
            <x-ui.section :title="__('settings.profile.password_title')" icon="lock" :noPadding="false">
                <form method="post" action="{{ route('password.update') }}" class="space-y-5">
                    ...
                    <div class="flex items-center gap-3 pt-2">
                        ...
                        @if (session('status') === 'password-updated')
                            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                                class="text-xs font-bold text-emerald-600">{{ __('common.updated_successfully') }}</p>
                        @endif
                    </div>
                </form>
            </x-ui.section>
            ...
            <x-ui.section :title="__('settings.profile.danger_zone')" icon="alert-triangle" :noPadding="false">
                <div class="p-4 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-200 dark:border-red-800">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <p class="text-sm font-bold text-red-700 dark:text-red-400">{{ __('settings.profile.delete_account') }}</p>
                            <p class="text-xs text-red-600/70 dark:text-red-300/70 mt-1">
                                {{ __('settings.profile.delete_account_desc') }}
                            </p>
                        </div>
                ...
                <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
                    <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <h2 class="text-lg font-bold text-heading">{{ __('settings.profile.delete_account_confirm') }}</h2>
                        </div>
                        ...
                        <div class="mb-6">
                            <input name="password" type="password" placeholder="{{ __('settings.profile.current_password') }}"
